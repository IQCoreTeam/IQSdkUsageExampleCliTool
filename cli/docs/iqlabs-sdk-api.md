...
Signature: `codein(input: { connection: Connection; signer: Signer }, chunks: string[], isAnchor?: boolean, filename?: string, method?: number, filetype?: string): Promise<string>`
...
### constructor
Signature: `new ReplayServiceClient(config?: { replayBaseUrl?: string; headers?: Record<string, string>; fetcher?: typeof fetch })`

### enqueueReplay
Signature: `enqueueReplay(request: { sessionPubkey: string }): Promise<{ jobId: string; status: string; retryAfter?: number; estimatedWaitMs?: number }>`

### getReplayStatus
Signature: `getReplayStatus(jobId: string): Promise<{ jobId: string; status: string; error?: string; chunkStats?: Record<string, unknown>; hasArtifact: boolean; downloadUrl?: string }>`
