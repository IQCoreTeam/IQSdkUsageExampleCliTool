...
import { GIT_CONSTANTS, Repository, Commit, FileTree } from "./types";
...
export class GitService {
  ...
  async createBranch(repoName: string, branchName: string, commitId: string) {
    ...
    const tableSeed = sha256(GIT_CONSTANTS.REFS_TABLE);
  ...
  async canReadRepo(repoName: string): Promise<boolean> {
    const repos = await this.listRepos();
    const repo = repos.find((r) => r.name === repoName);
    ...
    const collabs = await this.getCollaborators(repoName);
    return collabs.some((c) => c.userAddress === myAddr);
  }

  async canWriteToRepo(repoName: string): Promise<boolean> {
    const repos = await this.listRepos();
    const repo = repos.find((r) => r.name === repoName);
  ...
  async addCollaborator(repoName: string, userAddress: string) {
    ...
    const tableSeed = sha256(GIT_CONSTANTS.COLLABORATORS_TABLE);
  ...
  async getCollaborators(repoName: string): Promise<any[]> {
    const tableSeed = sha256(GIT_CONSTANTS.COLLABORATORS_TABLE);
    const dbRoot = iqlabs.contract.getDbRootPda(this.dbRootId, this.programId);
    const table = iqlabs.contract.getTablePda(
      dbRoot,
  ...
  async checkout(commitId: string, outputDir: string = "./checkout_output") {
  ...
  async clone(repoName: string, outputDir: string) {
    await this.ensureInfrastructure();
    ...
    await this.checkout(latest.id, outputDir);
  }
  ...
  async run(repoName: string) {
    ...
    await this.checkout(latest.id, tmpDir);
